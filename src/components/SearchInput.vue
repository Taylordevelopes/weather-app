  ...
  searchTerm.timeout = setTimeout(async () => {
    if (searchTerm.query !== '') {
      const res = await fetch(
        `http://api.weatherapi.com/v1/search.json?key=aef1a54c2680439691f11405242902&q=${searchTerm.query}`
      )
...
const getWeather = async (id) => {
  const res = await fetch(
    `http://api.weatherapi.com/v1/forecast.json?key=aef1a54c2680439691f11405242902&q=id:${id}&days=3&aqi=no&alerts=no`
  )
  const data = await res.json()
